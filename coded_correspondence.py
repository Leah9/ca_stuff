...
msg = "vhfinmxkl atox kxgwxkxw tee hy maxlx hew vbiaxkl tl hulhexmx. px'ee atox mh kxteer lmxi ni hnk ztfx by px ptgm mh dxxi hnk fxlltzxl ltyx."
for i in range(1,25):
    out = decoder(msg, i)
    if out.find("to") != -1 or out.find("and")!= -1:
        #print("find ", out.find("to"))
        print("offset is ", i)
        print(out)
        #break
...
msg = "barry is the spy"
key = "dog"

def ver_decode(msg, key):
    long_key = ""
    out = ""
    offset = 0
    count = 0
    temp = 0
    while len(msg) > len(long_key):
        long_key = long_key + key
    #print(long_key)
    for letter in msg:
        if letter in alphabet:
            temp=(alphabet.index(letter))
            temp1 = alphabet.index(long_key[count])
            offset = temp + temp1
            if offset > 25:
                offset -= 26
            #print("Letter,",letter, "Msg", temp, "Key", temp1, "offset ", offset)
            out = out + alphabet[offset]
       
            count += 1
        else:
            out = out + " "
    return out
print(ver_decode(msg, key))
...
msg = "this is taking me way longer than it should"
key = "bunny"
my_encoded = ver_encode(msg, key)
print("My encoded: ", my_encoded)
my_decoded = ver_decode(my_encoded, key)
print("My decoded: ", my_decoded)
